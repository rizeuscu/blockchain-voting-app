...
impl ContractInteract {
    async fn new() -> Self {
        ...
        let contract_code = BytesValue::interpret_from(
            "mxsc:../output/blockchain-voting-app.mxsc.json",
    ...
    async fn add_allowed_voter(&mut self) {
    ...
    async fn create_election(&mut self) {
        let name = ManagedBuffer::new_from_bytes(&b""[..]);
        let candidates = ManagedVec::from_single_item(ManagedBuffer::new_from_bytes(&b""[..]));
        let start = 0u64;
        let end = 0u64;
        ...
        let response = self
            ...
            .gas(30_000_000u64)
            .typed(proxy::BlockchainVotingAppProxy)
            .create_election(name, candidates, start, end)
            .returns(ReturnsResultUnmanaged)
            .prepare_async()
            .run()
            .await;
    ...
    async fn vote(&mut self) {
        let election_code = BigUint::<StaticApi>::from(0u128);
        let voted_candidate = ManagedBuffer::new_from_bytes(&b""[..]);
        ...
        let response = self
            ...
            .gas(30_000_000u64)
            .typed(proxy::BlockchainVotingAppProxy)
            .vote(election_code, voted_candidate)
            .returns(ReturnsResultUnmanaged)
            .prepare_async()
            .run()
            .await;
    ...
    async fn get_election_results(&mut self) {
        let election_code = BigUint::<StaticApi>::from(0u128);
        ...
        let result_value = self
            ...
            .to(self.state.current_address())
            .typed(proxy::BlockchainVotingAppProxy)
            .get_election_results(election_code)
            .returns(ReturnsResultUnmanaged)
            .prepare_async()
            .run()
            .await;
    ...
    async fn elections_metadata(&mut self) {
        let result_value = self
            .interactor
            .query()
            .to(self.state.current_address())
